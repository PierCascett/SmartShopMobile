const express = require('express');
const router = express.Router();
const db = require('../db');

// GET /api/scaffali - lista scaffali con conteggio prodotti
router.get('/', async (req, res) => {
    try {
        const result = await db.query(
            `SELECT s.id_scaffale, s.nome, s.descrizione,
                    COUNT(c.id_catalogo) AS prodotti_count
             FROM scaffali s
             LEFT JOIN catalogo c ON c.id_scaffale = s.id_scaffale
             GROUP BY s.id_scaffale, s.nome, s.descrizione
             ORDER BY s.id_scaffale`
        );
        res.json(result.rows);
    } catch (error) {
        console.error('Errore nel recupero degli scaffali:', error);
        res.status(500).json({ error: 'Errore nel recupero degli scaffali' });
    }
});

// GET /api/scaffali/:id - dettaglio scaffale
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    if (!/^[0-9]+$/.test(id)) {
        return res.status(400).json({ error: 'ID scaffale non valido' });
    }
    try {
        const result = await db.query(
            `SELECT s.id_scaffale, s.nome, s.descrizione,
                    COUNT(c.id_catalogo) AS prodotti_count
             FROM scaffali s
             LEFT JOIN catalogo c ON c.id_scaffale = s.id_scaffale
             WHERE s.id_scaffale = $1
             GROUP BY s.id_scaffale, s.nome, s.descrizione`, [id]
        );
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Scaffale non trovato' });
        }
        res.json(result.rows[0]);
    } catch (error) {
        console.error('Errore nel recupero dello scaffale:', error);
        res.status(500).json({ error: 'Errore nel recupero dello scaffale' });
    }
});

// GET /api/scaffali/:id/prodotti - prodotti per scaffale
router.get('/:id/prodotti', async (req, res) => {
    const { id } = req.params;
    if (!/^[0-9]+$/.test(id)) {
        return res.status(400).json({ error: 'ID scaffale non valido' });
    }
    try {
        const result = await db.query(
            `SELECT
                p.id_prodotto AS id,
                p.nome AS name,
                p.marca AS brand,
                p.id_categoria::text AS "categoryId",
                c.nome AS "categoryName",
                c.descrizione AS "categoryDescription",
                cat.id_catalogo AS "catalogId",
                cat.quantita_disponibile AS "catalogQuantity",
                cat.prezzo AS price,
                cat.vecchio_prezzo AS "oldPrice",
                cat.id_scaffale AS "shelfId",
                COALESCE(SUM(mg.quantita_disponibile), 0) AS "warehouseQuantity",
                COALESCE(SUM(mg.quantita_disponibile), 0) + cat.quantita_disponibile AS "totalQuantity",
                CASE
                    WHEN cat.quantita_disponibile <= 0 THEN 'Non disponibile'
                    WHEN cat.quantita_disponibile <= 5 THEN 'Quasi esaurito'
                    ELSE 'Disponibile'
                END AS availability,
                COALESCE(
                    json_agg(DISTINCT t.nome) FILTER (WHERE t.nome IS NOT NULL),
                    '[]'
                ) AS tags,
                p.descrizione AS description
             FROM prodotti p
             JOIN catalogo cat ON cat.id_prodotto = p.id_prodotto
             LEFT JOIN magazzino mg ON mg.id_prodotto = p.id_prodotto
             LEFT JOIN prodotti_tag pt ON pt.id_prodotto = p.id_prodotto
             LEFT JOIN tag t ON t.id_tag = pt.id_tag
             LEFT JOIN categorie_prodotti c ON c.id_categoria = p.id_categoria
             WHERE cat.id_scaffale = $1
             GROUP BY p.id_prodotto, p.nome, p.marca, p.id_categoria, p.descrizione,
                      cat.id_catalogo, cat.quantita_disponibile, cat.prezzo, cat.vecchio_prezzo,
                      cat.id_scaffale, c.nome, c.descrizione
             ORDER BY p.nome`, [id]
        );
        res.json(result.rows);
    } catch (error) {
        console.error('Errore nel recupero dei prodotti per scaffale:', error);
        res.status(500).json({ error: 'Errore nel recupero dei prodotti per scaffale' });
    }
});

module.exports = router;

